...
| **TTL / expiry** | `pending_tunnel_commands[i].created_at` üzerinden yaşı **> `TUNNEL_CMD_TTL`** (öneri: **24 saat**) olan komut `expired` işaretlenir ve yanıttan düşülür. |
| **Ack / consume** | Agent `POST /api/agent/tunnel-status` ile bir servisin gerçek durumunu bildirdiğinde, o servise ait `desired == reported` olan bekleyen komutlar **completed** sayılıp kuyruktan silinir. |
| **Dedupe** | Aynı `service` için birden çok bekleyen komut varsa yalnızca **en yenisi** tutulur; eskiler `superseded`. |
| **İdempotent** | Zaten `desired` ile aynı state’i isteyen komut kuyruğa yazılmaz. |
...
- [ ] Cloud: `tunnel-set` → `services[].desired` anında güncellenir
- [ ] Cloud: `created_at` > 24s bekleyen `pending_tunnel_commands` `expired` + yanıttan düşer
- [ ] Cloud: `desired == reported` olunca ilgili bekleyen komut `completed`/silinir
